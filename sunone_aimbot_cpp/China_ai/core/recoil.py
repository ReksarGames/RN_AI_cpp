...
class RecoilMixin:
    ...
    def update_mouse_re_ui_status(self):
        ...
        try:
            switch_text = 'ON' if self.config.get('recoil', {}).get('use_mouse_re_trajectory', False) and getattr(self, 'down_switch', False) else 'OFF'
            ...
            if not isinstance(mapping, dict):
                print(f'[Warning] mapping is not dict type: {type(mapping)}, resetting to empty dict')
                mapping = {}
            ...
            if dpg.does_item_exist('mouse_re_switch_text'):
                dpg.set_value('mouse_re_switch_text', f'Switch: {switch_text}')
            if dpg.does_item_exist('mouse_re_file_text'):
                dpg.set_value('mouse_re_file_text', f'Mapping file: {file_text}')
            if dpg.does_item_exist('mouse_re_points_text'):
                dpg.set_value('mouse_re_points_text', f'Trajectory points: {points_cnt}')
    ...
    def on_use_mouse_re_trajectory_change(self, sender, app_data):
        ...
        try:
            ...
            print(f"[mouse_re] Trajectory recoil: {('enabled' if enabled else 'disabled')}")
    ...
    def on_import_mouse_re_trajectory_click(self, sender, app_data):
        ...
        try:
            ...
            root = tk.Tk()
            root.withdraw()
            file_path = filedialog.askopenfilename(title='Select mouse_re trajectory JSON', filetypes=[('JSON files', '*.json'), ('All files', '*.*')])
            root.destroy()
            if not file_path:
                return
    ...
    def _parse_mouse_re_json(self, path):
        ...
        try:
            ...
            if not isinstance(data, dict):
                print(f'[mouse_re] JSON format error: root object is not dict, is {type(data)}')
                return
            ...
            print(f'[mouse_re] Successfully parsed trajectory: {len(points)} incremental points')
            return points
        except Exception as e:
    ...
    def render_mouse_re_games_combo(self):
        ...
        try:
            ...
            self.mouse_re_games_combo = dpg.add_combo(label='mouse_re Game', items=games, default_value=self.mouse_re_picked_game, callback=self.on_mouse_re_games_change, width=150, parent='mouse_re_combos_group')
        except Exception as e:
    ...
    def render_mouse_re_guns_combo(self):
        ...
        try:
            ...
            if not self.mouse_re_picked_game:
                self.mouse_re_guns_combo = dpg.add_combo(label='mouse_re Gun', items=[], default_value='', callback=self.on_mouse_re_guns_change, width=150, parent='mouse_re_combos_group')
                return
            games_config = self.config.get('games', {})
            ...
            self.mouse_re_guns_combo = dpg.add_combo(label='mouse_re Gun', items=guns, default_value=self.mouse_re_picked_gun, callback=self.on_mouse_re_guns_change, width=150, parent='mouse_re_combos_group')
        except Exception as e:
