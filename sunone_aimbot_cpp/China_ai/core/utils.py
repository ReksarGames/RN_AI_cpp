...
def check_tensorrt_availability():
    ...
    try:
        ...
        if not found_nvinfer:
            print('TensorRT DLL files not found, will use ONNX inference')
            return False
        try:
            import tensorrt as trt
            import pycuda.driver as cuda
            cuda.init()
            print('TensorRT environment detected successfully')
            return True
        except (ImportError, OSError) as e:
            print(f'TensorRT module import failed: {e}')
            print('Will use ONNX inference')
            return False
    except Exception as e:
        print(f'TensorRT environment detection failed: {e}')
        print('Will use ONNX inference')
        return False
...
def global_exception_hook(exctype, value, tb):
    with open('error_log.txt', 'a', encoding='utf-8') as f:
        ...
        f.write('\n')
    print('Program encountered uncaught exception, details written to error_log.txt. Please report this file to developers.')
...
if hasattr(threading, 'excepthook'):
    def thread_exception_hook(args):
        with open('error_log.txt', 'a', encoding='utf-8') as f:
            ...
            f.write('\n')
        print('Child thread encountered uncaught exception, details written to error_log.txt. Please report this file to developers.')
