...
class FlashbangMixin:
    ...
    def on_auto_flashbang_enabled_change(self, sender, app_data):
        if not self.is_using_dopa_model():
            print('Auto flashbang feature only supports ZTX models, current model not supported')
    ...
    def on_flashbang_debug_info(self, sender, app_data):
        ...
        print(f"Feature status: {('enabled' if self.config['auto_flashbang']['enabled'] else 'disabled')}")
        dopa_status = self.is_using_dopa_model()
        print(f"ZTX model status: {('loaded' if dopa_status else 'not used/not loaded')}")
        if hasattr(self, 'group') and self.group:
            current_model = self.config['groups'][self.group].get('infer_model', '')
        print(f"Feature availability: {('available' if self.config['auto_flashbang']['enabled'] and dopa_status else 'unavailable')}")
        print('Configuration parameters:')
        for key, value in self.config['auto_flashbang'].items():
            print(f'  {key}: {value}')
        print(f'Last trigger time: {time.time() - self.last_flashbang_time:.1f} seconds ago')
    ...
    def detect_and_handle_flashbang(self, boxes, class_ids, model_width, model_height, scores=None):
            ...
            if class_id == 4:
                ...
                try:
                    ...
                    if width == 0 or height == 0:
                        print(f'  Warning: Abnormal detection box size width={width}, height={height}')
                        continue
                except (IndexError, TypeError) as e:
                    print(f'  Error: Abnormal box data format {e}')
                    continue
                print(f'Found class 4: confidence={current_confidence:.3f}, size={width:.1f}x{height:.1f}, min_edge={min_box_size:.1f}')
                if scores is not None and scores[i] < min_confidence:
                    print(f'  Skip: confidence {current_confidence:.3f} < {min_confidence}')
                else:
                    if min_box_size < min_size:
                        print(f'  Skip: size {min_box_size:.1f} < {min_size} (distant flashbang may need to lower this threshold)')
                    else:
                        print('  Passed check, adding to flashbang list')
                        flashbang_indices.append(i)
        if class4_detected and (not flashbang_indices):
            print('Detected class 4 but all filtered - suggest lowering minimum confidence or size threshold')
        if not flashbang_indices:
            return
        print(f'Detected {len(flashbang_indices)} valid flashbangs, preparing to execute evasion')
        left_count = 0
        right_count = 0
        ...
        for idx in flashbang_indices:
            ...
            try:
                ...
                print(f'Flashbang position: x={flashbang_center_x:.1f} (relative position: {relative_pos:.2f})')
        ...
        print(f'Flashbang distribution: {left_count} on left, {right_count} on right, evading to {direction_text}')
    ...
    def execute_flashbang_turn(self, turn_direction):
        ...
        try:
            ...
            print(f'Actual movement distance: X={self.flashbang_actual_move_x}, Y={self.flashbang_actual_move_y}')
    ...
    def execute_flashbang_return(self, original_turn_direction):
        ...
        try:
            try:
                ...
                print('Executing precise return: X=' + f'{return_move_x}' + ', Y=' + f'{return_move_y}' + ' pixels (based on actual movement distance)')
                if self.config['auto_flashbang']['use_curve']:
    ...
    def execute_flashbang_curve_move(self, relative_move_x, relative_move_y):
        ...
        try:
            ...
            if isinstance(curve_points, tuple):
                ...
                print(f'Flashbang curve control points: {knots_count}, generated trajectory points: {len(curve_points)}')
                ...
                if abs(total_x - target_x) > 1 or abs(total_y - target_y) > 1:
                    print(f'Warning: Curve movement total distance mismatch, target:({target_x},{target_y}), actual:({total_x:.2f},{total_y:.2f})')
                ...
                for i, (dx, dy) in enumerate(frame_moves):
                    ...
                    if i < total_frames - 1:
                        ...
                        pass
                print(f'Flashbang curve movement complete, total frames: {total_frames}, total time: {frame_time * total_frames:.3f}s')
        except Exception as e:
    ...
    def execute_flashbang_curve_move_fast(self, relative_move_x, relative_move_y):
        ...
        try:
            ...
            if isinstance(curve_points, tuple):
                ...
                print(f'Return fast curve: {knots_count} control points, trajectory points: {len(curve_points)}')
                total_distance = math.sqrt(relative_move_x ** 2 + relative_move_y ** 2)
                base_duration = 0
                frame_count = 1
                if len(curve_points) < frame_count:
                ...
                for i, (dx, dy) in enumerate(frame_moves):
                    ...
                    if move_x == 0 and move_y == 0:
                        continue
                    self.move_r(move_x, move_y)
                print(f'Fast return complete, total frames: {total_frames}, total time: {frame_time * total_frames:.3f}s')
        except Exception as e:
    ...
    def execute_flashbang_ultra_fast_move(self, relative_move_x, relative_move_y):
        ...
        """
        try:
            print(f'Ultra-fast flashbang movement: X={relative_move_x}, Y={relative_move_y}')
            total_distance = math.sqrt(relative_move_x ** 2 + relative_move_y ** 2)
            if total_distance <= 100:
                ...
                if total_distance <= 500:
                    ...
                    print(f'Medium distance 2-step movement complete: ({step1_x},{step1_y}) -> ({step2_x},{step2_y})')
                else:
                    ...
                    print(f'Large distance 3-step movement complete: ({step1_x},{step1_y}) -> ({step2_x},{step2_y}) -> ({step3_x},{step3_y})')
            print(f'Ultra-fast movement complete, actual movement: X={actual_total_x}, Y={actual_total_y}')
            return (actual_total_x, actual_total_y)
        except Exception as e:
