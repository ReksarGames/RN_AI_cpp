"""Aiming, targeting, mouse control and trigger system mixin.
...
from catbox_wrapper import catbox_left_down, catbox_left_up
from infer_function import read_img, nms, nms_v8
    ...
    def infer(self):
        ...
        while self.running:
            ...
            if pred.ndim == 1:
                ...
                if pred.size % C!= 0:
                    raise ValueError(f'Inference output length {pred.size} cannot be divided by feature count per row {C}, please check model!')
            ...
            if self.config['auto_flashbang']['enabled'] and len(boxes) > 0 and (len(class_ids) > 0):
                ...
                if self.config['auto_flashbang']['enabled']:
                    if len(class_ids) > 0 and 4 in class_ids:
                        if self.is_using_dopa_model():
                            print('Detected class 4, but boxes is empty or filtered empty')
                        else:
                            ...
                            if self.makcu is not None:
                                ...
                                for retry in range(max_retries):
                                    try:
                                        ...
                                        if retry == max_retries - 1:
                                            print(f'Makcu release failed: {e}')
                                            try:
                                                self.makcu.disconnect()
                                                time.sleep(0.1)
                                                self.makcu.connect()
